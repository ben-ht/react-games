...
describe('GameDetails', () => {
	it('It renders', () => {
		...
		return;
		const result = render(<View game={game} screenshots={screenshots} />);

		expect(
			screen.getAllByText(new RegExp(game.name))[0],
		).toBeInTheDocument();
		expect(
			screen.getAllByText(new RegExp(game.storyline))[0],
		).toBeInTheDocument();

		const ratings = result.container.querySelectorAll('.game-ratings');
		assert(ratings !== undefined);

		const carousel = result.container.querySelector(
